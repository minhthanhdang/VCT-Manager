...
interface GenerateAttackCardProps {
  players: string[],
  agents: string[],
  map: string,
  sessionId: string
...
export const GenerateAttackCard = ({
  players,
  agents,
  map,
  sessionId
}: GenerateAttackCardProps) => {
  ...
  const onSubmit = async (map: string, agents: string[], players: string[]) => {
    ...
    try {
      setIsLoading(true)
      let prompt = "Generate a strategy for attacking on map " + map + " with the following agents: " + agents.join(", ") + " played by players: " + players.join(", ") + " respectively."
      console.log(prompt)
  ...
  return (
    <Card className="relative w-full h-full max-h-[300px] overflow-y-scroll scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-gray-900 pb-10">
      <CardHeader>
        <CardTitle className="text-[22px] leading-4">Offense strategy recommendation</CardTitle>
